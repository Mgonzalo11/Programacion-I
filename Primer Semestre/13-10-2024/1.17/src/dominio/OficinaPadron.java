...
import java.io.FileWriter; // Importa la clase FileWriter para escribir datos en archivos.
...
public class OficinaPadron {
    ...
    // Metodo para añadir un nuevo habitante al padrón y volcar los datos al archivo CSV.
    public void annadir(Habitante habitante) {
        habitantesPadron.add(habitante); // Añade el habitante a la lista de habitantes.
        volcarContactos(); // Llama al metodo para volcar la lista de habitantes al archivo CSV después de añadir el nuevo habitante.
    }
    ...
    // Metodo privado que volca los datos de la lista de habitantes al archivo CSV.
    private void volcarContactos() {
        try {
            // Crea un FileWriter para escribir en el archivo "padron.csv"
            FileWriter fw = new FileWriter("padron.csv");
            // Itera sobre cada habitante en la lista y escribe sus datos en el archivo.
            for (Habitante habitante : habitantesPadron) {
                fw.write(habitante.getNombre() + "," +
                        habitante.getApellido1() + "," +
                        habitante.getApellido2() + "\n"); // Separar los campos por comas y añadir salto de línea.
            }
